...
def num_resting_units(puzzle_input, fall_to_floor=False):
    ...
    while sand_y <= lowest_rock_y and sand_map[SAND_SOURCE] != SAND:
        if fall_to_floor and sand_y == lowest_rock_y - 1:
            # Right above the floor, can't fall lower
            sand_map[sand_x, sand_y] = SAND
            sand_x, sand_y = SAND_SOURCE
        elif (sand_x, sand_y + 1) not in sand_map:
            sand_y += 1
    ...
    return sum(obj == SAND for obj in sand_map.values())
