...
Path = namedtuple("Path", ["current_cave", "visited_caves"])
...
def get_num_paths(puzzle_input):
    edges = [line.split("-") for line in puzzle_input.split("\n")]
    outbound_edges = defaultdict(list)
    for x, y in edges:
        outbound_edges[x].append(y)

    potential_paths = [Path(current_cave=START, visited_caves=set([START]))]
    num_paths = 0
    while len(potential_paths) > 0:
        current_cave, visited_caves = potential_paths.pop()
        if current_cave == END:
            num_paths += 1
            continue
        ...
        for next_cave in outbound_edges[current_cave]:
            if _is_big_cave(next_cave) or next_cave not in visited_caves:
                ...
                potential_paths.append(
                    Path(current_cave=next_cave, visited_caves=new_visited_caves)
                )
...
if __name__ == "__main__":
    ...
    print(f"Part 2: {get_num_paths(puzzle_input)}")
