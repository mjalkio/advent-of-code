...
def get_outputs_sum(puzzle_input):
    ...
    output_sum = 0
    for entry in entries:
        unique_patterns, output_patterns = [digits.split(' ') for digits in entry.split(' | ')]
        ...
        five_segment_numbers = [pattern for pattern in unique_patterns if len(pattern) == 5]
        six_segment_numbers = [pattern for pattern in unique_patterns if len(pattern) == 6]
