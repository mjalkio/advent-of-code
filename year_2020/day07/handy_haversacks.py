...
def _get_bag_rules(puzzle_input):
    ...
    for line in lines:
        ...
        rules[bag_type] = {
            ...
            for num_bags, contained_bag_type in [
                contained_bag.split(" ", maxsplit=1)
                for contained_bag in contained_bag_details.split(", ")
            ]
        }
    return rules
...
def num_bags_contained_in_my_bag(puzzle_input, my_bag="shiny gold"):
    ...
    while my_bag not in num_bags_in:
        for bag, contained_bags in rules.items():
            if all([contained_bag in num_bags_in for contained_bag in contained_bags]):
                num_bags_in[bag] = sum(
                    contained_bag_quantity
                    + contained_bag_quantity * num_bags_in[contained_bag]
                    for contained_bag, contained_bag_quantity in contained_bags.items()
                )
