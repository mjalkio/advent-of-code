...
def get_field_order(puzzle_input):
    ...
    possible_fields = {
        position: list(rules.keys()) for position in range(len(my_ticket))
    }
    for position in range(len(my_ticket)):
        for ticket in valid_tickets:
            ...
            for field_name in list(possible_fields[position]):
                if not any(
                    position_value in valid_range for valid_range in rules[field_name]
                ):
    ...
    while any(len(fields) > 1 for fields in possible_fields.values()):
        known_fields = [
            fields[0] for fields in possible_fields.values() if len(fields) == 1
        ]
